...
# Ho inserito i valori esatti che mi hai fornito per i template "app"
TEMPLATE_MAPS = {
    ...
    # Formati Standard (Copertura totale con blending)
    ...
    "30x30-crea la tua grafica.jpg": (15.0, 15.0, 70.0, 70.0), # Calibrata per non essere un "poster"
}
...
with t_prod:
    ...
    if disegni and libreria[formato]:
        ...
        # Anteprime Totali (Griglia)
        st.divider()
        st.subheader(f"Anteprime per {formato}")
        grid = st.columns(2)
...
with t_sett:
    ...
    if libreria["Tutti"]:
        ...
        with c_ctrl:
            x = st.number_input("X Start %", value=def_coords[0], step=0.1)
            y = st.number_input("Y Start %", value=def_coords[1], step=0.1)
            w = st.number_input("Larghezza %", value=def_coords[2], step=0.1)
            h = st.number_input("Altezza %", value=def_coords[3], step=0.1)
