...
# --- 1. MAPPA COORDINATE DEFINITIVA ---
# Inclusi i formati standard con logica di blending (0,0,100,100)
TEMPLATE_MAPS = {
    ...
    # Formati Standard (Ora elaborati con Shadow Mapping e Blending)
...
def apply_mockup(tmpl_pil, cover_pil, coords, blur_rad=5.0):
    ...
    # Protezione dimensioni (evita crash su crop fuori bordo)
    tw, th = max(1, min(tw, w_f - x1)), max(1, min(th, h_f - y1))

    # Resize cover della foto caricata
    cover_res = np.array(cover_pil.convert('RGB').resize((tw, th), Image.LANCZOS)).astype(np.float64)
    
    # --- SHADOW MAP (Prende le ombre dal template originale) ---
    # Converte l'area del template in scala di grigi per usarla come mappa di moltiplicazione
    shadow_area = tmpl_img.crop((x1, y1, x1+tw, y1+th)).convert('L')
    shadow_map = np.array(shadow_area).astype(np.float64) / 255.0
    shadow_map = np.expand_dims(shadow_map, axis=2)

    # --- MASCHERA DI SFUMATURA (Bordi morbidi) ---
    mask = Image.new("L", (tw, th), 255)
    if blur_rad > 0:
        draw = ImageDraw.Draw(mask)
        # Disegna un rettangolo nero leggermente più piccolo per creare il gradiente
    ...
    # Moltiplica la tua foto per le ombre del template (Effetto "stampa su carta")
    cover_with_shadows = cover_res * shadow_map
    
    # Fonde la foto con il template originale usando la maschera alpha per i bordi
...
st.set_page_config(page_title="PhotoBook Master V7.6", layout="wide")
st.title("📖 PhotoBook Master V7.6")
...
with t_prod:
    ...
    with col_l:
        ...
        sfocatura = st.slider("Sfocatura Bordi (Fumetto):", 0.0, 10.0, 5.0, 0.5)
        
    with col_r:
        disegni = st.file_uploader("Carica le tue grafiche (JPG/PNG):", accept_multiple_files=True)
    ...
    if disegni and libreria[formato]:
        if st.button("🚀 GENERA TUTTI I MOCKUP"):
            ...
            with zipfile.ZipFile(z_io, "a") as zf:
                for d in disegni:
                    d_img = Image.open(d)
                    # Per ogni grafica caricata, genera un mockup per ogni template nella categoria
                    for t_n, t_i in libreria[formato].items():
                        ...
                        # Salva in cartelle separate per ogni file caricato
                        zf.writestr(f"{os.path.splitext(d.name)[0]}/{t_n}", buf.getvalue())
            
            st.success("Generazione completata!")
            st.download_button("📥 SCARICA ZIP MOCKUPS", z_io.getvalue(), "Mockups_Batch.zip")
        
        # Anteprime Rapide
        st.divider()
        st.subheader(f"Anteprime per categoria {formato} (Ultima immagine caricata)")
        grid = st.columns(2)
        for i, (name, img) in enumerate(libreria[formato].items()):
            with grid[i % 2]:
                coords = TEMPLATE_MAPS.get(name, (0, 0, 100, 100))
                # Mostra l'anteprima usando l'ultimo file caricato
                st.image(apply_mockup(img, Image.open(disegni[-1]), coords, sfocatura), caption=name)

with t_sett:
    st.subheader("⚙️ Regolazione Fine Coordinate")
    if libreria["Tutti"]:
        t_sel = st.selectbox("Seleziona Template da calibrare:", list(libreria["Tutti"].keys()))
        ...
        with c_ctrl:
            ...
            t_test = st.file_uploader("Immagine di test per calibrazione:", type=['jpg', 'png'], key="test_uploader")
            
            st.info("Copia questa riga in TEMPLATE_MAPS:")
        ...
        with c_view:
            if t_test:
                ...
                st.warning("Carica un'immagine di test per vedere l'anteprima della calibrazione.")
