...
# --- 1. CONFIGURAZIONE ---
st.set_page_config(page_title="PhotoBook PRO - Classic", layout="wide")

if 'uploader_key' not in st.session_state:
    st.session_state.uploader_key = 0

# --- 2. LOGICA SMISTAMENTO TEMPLATE (Base su nome file) ---
...
# --- 3. LOGICA GENERAZIONE (TORNATA ALL'ORIGINALE) ---
def find_book_region(tmpl_gray, bg_val):
    # Logica standard senza espansioni (abbondanza)
    h, w = tmpl_gray.shape
    ...
    # Restituisce le coordinate esatte rilevate
...
def process_image(tmpl_pil, cover_pil):
    ...
    # --- LOGICA ORIGINALE: ADATTAMENTO ESATTO ---
    # Nessun margine, nessun bordo smart. Adatta l'immagine all'area rilevata.
    fw, fh = reg['w'], reg['h']
    cover_rgb = cover_pil.convert('RGB')
    resized = ImageOps.fit(cover_rgb, (fw, fh), Image.LANCZOS)
    cover_res = np.array(resized).astype(np.float64)
    
    # Calcolo colore dorso standard
